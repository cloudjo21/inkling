...
class RegisterIncrementalItemAggrInfoSchema(RegisterSchema):
    def __init__(self, service_config, task_config):
        super(RegisterIncrementalItemAggrInfoSchema, self).__init__(service_config, task_config)
...
if __name__ == "__main__":
    ...
    for deploy_env in deploy_envs:
        ...
        dag_config = YamlLoader(f"{service_config.resource_path}/item-aggr-info/incremental/dag.yml").load()
        task_config = get_my_task(
            dag_config=dag_config, 
            context=None, 
            dag_id="dag_load_incremental_item_aggr_info", 
            task_id="register_incremental_item_aggr_info_schema"
        )
        RegisterIncrementalItemAggrInfoSchema(service_config, task_config)()
