...
class LoadIndexUserRequestReco:
    ...
    def __call__(self, context):
        ...
        index_template_path = f"{service_config.resource_path}/index_template/item_matching/user_request_reco/index_template.json"
        with open(index_template_path, mode="r") as f:
            index_template = json.load(f)
            # create_index(es=self.search_client, index_name=recent_index_name, mappings=index_template["mappings"])
        ...
        vector_fetcher.fetch(vector_writer, ingress_latest_path)
...
if __name__ == "__main__":
    ...
    dag_config_filepath = f"{service_config.resource_path}/item-matching/dag-build-index-item-reco/dag.yml"
    dag_config = YamlLoader(dag_config_filepath).load()

    task_config = get_my_task(dag_config, context=None, dag_id='dag_build_index_reco', task_id='load_index_user_request_reco')

    task = LoadIndexUserRequestReco(service_config, task_config)
    task(context={})
