...
class CalcFirstTermRefineInterval:
    ...
    def __call__(self, calc_req: CalcTermRefineIntervalRequest, udf_map: dict) -> float:
        ...
        first_terms_df = terms_df.withColumn('term', udf_map['first_term'](col('terms')))
        term_avg = first_terms_df.agg(avg('term').alias('avg')).collect()[0].avg
        term_stddev = first_terms_df.agg(stddev('term').alias('stddev')).collect()[0].stddev
        term_min = first_terms_df.where('term < 0.0').agg(min('term').alias('min')).collect()[0].min
        term_max = first_terms_df.where('term < 0.0').agg(max('term').alias('max')).collect()[0].max
...
class CalcFirstTermRefineIntervalObject:
    @classmethod
    def apply(cls, calc_req, refinery, service_config):
        ...
        calc = CalcFirstTermRefineInterval(service_config)
        udf_map = {
...
class CalcFirstTermRefineIntervalJob:
    ...
    def apply(cls, service_config, schema_type='keyword', source_type='mix', min_keyword_length=3) -> dict:
        ...
        for kwd_len in list(range(min_keyword_length, max_keyword_length+1)):
            ...
            lower_bound_threshold = CalcFirstTermRefineIntervalObject.apply(calc_req, refinery, service_config)
        ...
        first_term_refine_interval_path = f"/user/{service_config.username}/lake/first_term_refine_threshold/{schema_type}/{source_type}/{snapshot_now()}"
        file_service.mkdirs(first_term_refine_interval_path)
        file_service.write(first_term_refine_interval_path + "/" + "data.json", json.dumps(len2thld, ensure_ascii=False))
